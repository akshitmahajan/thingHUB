<html>
 <head>
   <LINK REL=StyleSheet HREF="ss.css" TYPE="text/css" MEDIA=screen>
   <LINK REL=StyleSheet HREF="button.css" TYPE="text/css" MEDIA=screen>
</head>
<%@page import="java.sql.*"%>


<%

        Connection c = null;
        Statement stmt = null;
        ResultSet rs = null;
%>

        <table align="center"cellpadding="4" cellspacing="4" >
        
        <%
                try
                {
                                Class.forName("com.mysql.jdbc.Driver");

        c = DriverManager.getConnection("jdbc:mysql://localhost:3306/Mapping","root","raspberry");
                        stmt=c.createStatement();

                        String sql ="SELECT Zone,ResourceType FROM IP;";

                        rs = stmt.executeQuery(sql);

            while(rs.next()){

        %>
        <tr>
        <form action="toggle" method="post">
        <input type="hidden" name ="toggle" value=<%=rs.getString("ResourceType") %> >
        <input type="hidden" name ="toggle1" value=<%=rs.getString("Zone") %> >

        <td><button type="submit" class="btn btn-2 btn-2j"><%=rs.getString("Zone")%> <%=rs.getString("ResourceType") %></button></td>
         </form>
        </tr>

        <%
                        }

        } catch (Exception e)
        {
                e.printStackTrace();
        }


%>
</table>

</html>
